...
if [[ -f "$basedir/${FORK_NAME}-API/.git/patch-apply-failed" ]]; then
    echo "$(bashColor 1 31)[[[ WARNING ]]] $(bashColor 1 33)- Not saving ${FORK_NAME}-Server as it appears ${FORK_NAME}-API did not apply clean.$(bashColorReset)"
    ...
    echo "$(bashColor 1 33)Otherwise, rerun ./dragon patch to have a clean PaperDragon-API apply so the latest PaperDragon-Server can build.$(bashColorReset)"
else
    savePatches ${FORK_NAME}-Server server
    [[ "$2" = "push" ]] && ${basedir}/scripts/push.sh
fi
